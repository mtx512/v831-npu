...
void nna_bias_relu() {
  ...
  uint8_t bias = 1;  // Bias value
	...
	mem_alloc(&input_d_mem, 0x1000);

	mem_init(&output_d_mem);
	mem_alloc(&output_d_mem, 0x1000);

  memset(input_d_mem.data_addr,0,0x1000);
  memset(output_d_mem.data_addr,0,0x1000);
  ...
  for (int i=0;i<32;i++) {
    ((int8_t *)input_d_mem.data_addr)[i*2] = i*4; //32767;
    ((int8_t *)input_d_mem.data_addr)[(i*2)+1] = -i*4; //32767;
  }
  ...
  dma_loadin((char*)input_d_mem.data_addr, 0x1000, (uint32_t)(gp_paddr));
  dma_loadin((char*)output_d_mem.data_addr, 0x1000, ((uint32_t)(gp_paddr)+2048));

  memset(input_d_mem.data_addr,0,0x1000);
  dma_loadout((uint32_t)gp_paddr, 0x1000, (char*)input_d_mem.data_addr);
  printf("input >>>>>>> \n");
  for (int j=0;j<16;j++) {
    printf("{");
    for (int i=0;i<8;i++) {
  ...
  xregw(0x8040,0x13);                      // SDP_RDMA_D_NRDMA_CFG_0   NRDMA_DATA_MODE=PER_ELEMENT, NRDMA_DATA_SIZE=TWO_BYTE, NRDMA_DATA_USE=ALU, NRDMA_DISABLE=YES
  xregw(0x8058,0x13);                      // SDP_RDMA_D_ERDMA_CFG_0   ERDMA_DATA_MODE=PER_ELEMENT, ERDMA_DATA_SIZE=TWO_BYTE, ERDMA_DATA_USE=ALU, BRDMA_DISABLE=YES

  xregw(0x9004,0x0);                       // SDP_S_POINTER_0
  xregw(0x9008,0x0);                       // SDP_S_LUT_ACCESS_CFG_0
  xregw(0x900c,0x0);                       // SDP_S_LUT_ACCESS_DATA_0
  xregw(0x9010,0x0);                       // SDP_S_LUT_CFG_0
  ...
  dma_loadout(((uint32_t)gp_paddr)+2048, 0x1000, (char*)output_d_mem.data_addr);
  printf("output >>>>>>> \n");
  for (int j=0;j<16;j++) {
    printf("{");
    for (int i=0;i<8;i++) {
